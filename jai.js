...
const STDLIB = {
	...
	'built_in.stdLib.Thekla_Atlas': [
		'atlas_set_message_callback',
		'atlas_set_assert_callback',
		'atlas_set_default_options',
		'atlas_generate',
		'atlas_free',
	],
	'type.stdLib.Thekla_Atlas': [
		'Atlas_Charter',
		'Atlas_Mapper',
		'Atlas_Packer',
		'Atlas_Options',
		...
		'Atlas_Error_Subtype',
		'Atlas_Error_Data',
		'Atlas_Message_Callback',
		'Atlas_Assert_Callback',
	],
	'built_in.stdLib.Thekla_Baker': [
		'baker_init',
		'baker_shut',
		'baker_start',
		'baker_get_total_number_of_samples',
		'baker_get_hemicube_viewport',
		'baker_render_done',
		'baker_progress',
		'baker_get_current_mesh_sample_count',
	],
	'type.stdLib.Thekla_Baker': [
		'Baker_Sampling_Mode',
		'Baker_Input_Info',
		'Baker_Input_Mesh',
		'Baker_Input_Points',
		'Baker_Input_Transform',
		'Baker_Frame',
		'Baker_Lightmap',
		'Baker_Options',
		'Log_Print',
	...
	'variable.stdLib.Vulkan.moduleParam': [
		'USE_VULKAN_1_1',
	],
	...
	'variable.stdLib.freetype-2.constant': [
		'TTAG_0xA5kbd',
		'TTAG_0xA5lst',
	],
	'built_in.stdLib.freetype255': [
		'FT_Init_FreeType',
		'FT_New_Face',
		'FT_New_Memory_Face',
		'FT_Load_Char',
	...
	'variable.stdLib.freetype255.constant': [
		'FT_Err_Ok',
	...
	'variable.stdLib.stb_vorbis.constant': [
...
const keywords =  {
	$pattern: /\b(?:#Context|[Aa-filnNpr-tTu-x][_1-468ac-ik-pr-z]+)\b/,
	'keyword.if': [
		'then|5',
		'else',
		'case',
...
const COMMENTS = [
	fixDocTags(hljs.COMMENT('//', /$/)),
	fixDocTags(hljs.COMMENT(/\/\*/, /\*\//, { contains: ['self'] }))
];
...
const HERESTRING = {
	...
	beginScope: {
		...
		2: 'meta.directive',
		3: 'punctuation.comma',
		4: 'meta.directive.modifier',
		6: 'meta.stringTerminator'
	},
	'on:begin': (m, resp) => resp.data.end = m[6],
	end: /^\s*(\w+)\b/,
...
const NUMBER = {
	scope: 'number',
	begin: /\b-?(?:(?:\d+(?:(?!\.)|(?=\.\.))|\d*\.\d+)|0[hx][\dA-Fa-f_]+|0b[01_]+)\b/,
	variants: [
		{
			scope: 'number.binary',
			begin: /\b-?0b[01_]+\b/
		...
		{
			scope: 'number.hex',
			begin: /\b-?0x[\dA-Fa-f_]+\b/
		},
		{
			scope: 'number.hexFloat',
			begin: /\b-?0h[\dA-Fa-f_]+\b/
		},
		{
			scope: 'number',
			begin: /\b-?(?:\d+(?!\.)|\d*\.\d+)/
...
//FIXME: proc: handle #modify and also other directives like #no_abc etc
...
const DIRECTIVE = {
	...
	begin: [
		/#/,
		/(?:add_context|align|as(?:m|sert|)|bake(?:_arguments|)|bytes|c_call|caller_location|char|code|compile_time|compiler|complete|cpp_method|define|deprecated|dump|else|endif|exists|expand|file(?:path|)|foreign(?:(?:_system)?_library|)|if(?:n?def|)|insert|intrinsic|library|load|location|modify|module_parameters|must|no_(?:abc|alias|aoc|context|debug|padding|reset)|place(?:holder|)|poke_name|procedure_name|program_export|run(?:time_support|)|scope_(?:export|file|module)|specified|sy(?:mmetric|stem_library)|th(?:is|rough)|type(?:_info_(?:procedures_are_void_pointers|none|v2)|))/,
	],
	beginScope: {
		1: 'punctuation.hash.directive',
		2: 'meta.directive'
...
//TODO: #module_parameters: handle like #import but with no path.

//TODO: explicit forExpansion name handling

//TODO: asm: +keyword, handle block as subLanguage

const PROC_DECLARATION = {
	scope: 'title.function.declaration',
	begin: `${identifierRE}(?=\\s*${skipCommentsRE}::\\s*${skipCommentsRE}(?:(?:no_)?inline\\s*${skipCommentsRE})?\\()`,
	returnBegin: true,
	keywords,
	contains: [ALIGNMENT_WS],
	end: /(?=\s*::)/
...
const ENUM_DECLARATION = {
	...
	begin: `${identifierRE}(?=\\s*${skipCommentsRE}::\\s*${skipCommentsRE}enum(?:_flags)?)`,
	returnBegin: true,
	keywords,
	contains: [
		ALIGNMENT_WS,
...
const STRUCT_DECLARATION = {
	...
	begin: `${identifierRE}(?=\\s*${skipCommentsRE}::\\s*${skipCommentsRE}struct)`,
	returnBegin: true,
	keywords,
	contains: [ALIGNMENT_WS],
	end: /(?=\s*::)/
